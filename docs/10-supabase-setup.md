...
## Шаг 4.5. Настраиваем ссылки в письмах (ОБЯЗАТЕЛЬНО для сброса пароля)

По умолчанию Supabase отправляет в письмах ссылки на `http://localhost:3000` — этот адрес существует только на компьютере разработчика, и по нему ничего не открывается (ошибка ERR_CONNECTION_REFUSED). Нужно один раз указать настоящий адрес сайта.

1. Откройте настройки: **https://supabase.com/dashboard/project/bwyytdjyskttxbqtznnh/auth/url-configuration** (или вручную: Authentication → URL Configuration).
2. **Site URL** — замените на:
   ```
   https://emilahayeu-max.github.io/Emil-s-Project-1
   ```
3. В блоке **Redirect URLs** нажмите **Add URL** и добавьте:
   ```
   https://emilahayeu-max.github.io/Emil-s-Project-1/**
   ```
   (две звёздочки в конце обязательны — они означают «любая страница этого сайта»).
4. Нажмите **Save**.

После этого письма (подтверждение почты, сброс пароля) будут открывать правильные страницы сайта.

**Если письмо со старой ссылкой уже пришло:** нажмите ссылку → в адресной строке браузера замените начало `http://localhost:3000` на `https://emilahayeu-max.github.io/Emil-s-Project-1` (остальное не трогайте) → Enter.
