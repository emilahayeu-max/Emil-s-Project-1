...
> **Статус:** ✅ Облако **подключено** (создан бесплатный проект Supabase пользователем по инструкции docs/10, ключи заведены в `web/.env.local` — файл вне git). Приложение работает в облачном режиме: `/api/health` отдаёт `"backend": "supabase"`.
...
✅ **SupabaseAdapter реализован** (`web/src/lib/adapters/supabase.ts`):
- сессии (вход/регистрация/выход) через Supabase Auth, подтверждение почты поддерживается;
- гидратация данных при старте (записи, задачи, логи дней, избранное, тема из профиля);
- оптимистичные записи с баннером об ошибках синхронизации;
- удаление аккаунта: каскадное стирание всех строк через RLS;
- мапперы БД → доменные типы покрыты юнит-тестами (`web/tests/adapter-map.test.ts`).

Переключение происходит автоматически при наличии env-ключей. Пошаговая инструкция создания бесплатного проекта Supabase для новичка — **[docs/10-supabase-setup.md](10-supabase-setup.md)**.

## 7. Архитектура доступа к данным (итог)
