...
| 11. Реализация MVP (код) | 🔨 В работе — `web/` (Next.js): каркас, i18n RU/EN, 2 темы, аккаунт, цикл дня, дневник, задачи, гид, настройки, напоминания. PWA: манифест, иконки, офлайн-режим. Схема Supabase + RLS в `supabase/`. **☁ Облако подключено** (ключи пользователя в `web/.env.local`, вне git) |
